...
export default function GoogleLoginButton({ onSuccess, onError }) {
  ...
  useEffect(() => {
    if (status === 'authenticated' && session && !isProcessing) {
      handleGoogleAuthSuccess(session)
    }
  }, [session, status, isProcessing])

  const handleGoogleAuthSuccess = async (session) => {
    try {
      setIsProcessing(true)
      
      // Call backend API to register/login user
      const result = await login(session.user.email, '', false)
  ...
  if (status === 'loading' || isProcessing) {
    return (
      ...
      >
        ...
        {isProcessing ? 'Processing...' : 'Signing in...'}
      </button>
    )
  }
  ...
  return (
    <button
      ...
      className="w-full flex items-center justify-center px-4 py-3 border border-gray-300 rounded-lg shadow-sm bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-200"
    >
